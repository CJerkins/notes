REF https://github.com/dumrauf/openvpn-terraform-install
...
After running the Terraform module in this repository you get
 ...
 - the option to provision and revoke users from the 
 - OpenVPN server by simply re-running the Terraform module 
...
shared_credentials_file can be created by running `awscli configure` in the terminal. The credentials file should look like as so,
```
[default]
aws_access_key_id = adkflaljeoadflka
aws_secret_access_key = jakljdfjijaeroadfasdf
```
`profile = "<your-profile>"` will most likely be labeled `default`
...
Initialise Terraform by running. Will need sudo or admin privlages
```
